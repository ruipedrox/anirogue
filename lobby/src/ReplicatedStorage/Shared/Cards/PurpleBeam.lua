...
local GALICK_SFX_ID = 137817798975468
...
local FORWARD_OFFSET_CHARGE = 0
local FORWARD_OFFSET_BEAM = 0
...
local function getGokuFolder()
	...
	return chars and chars:FindFirstChild("Vegeta_5") or nil
end
...
local function findNearestEnemy(fromPos: Vector3): Model?
...
function M.Start(player: Player)
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		local chargeTemplate = goku and goku:FindFirstChild("galick_charge")
		local beamTemplate = goku and goku:FindFirstChild("galick_gun")
		...
		-- Fire beam
		SFXHelper.playAt(hrp, GALICK_SFX_ID, 0.9, { minDist = 15, maxDist = 120, lifetime = durationNV.Value + 1 })
