...
local PURPLE_CHARGE_SFX_ID = 133183736968993
local PURPLE_FIRE_SFX_ID   = 80967988331526
...
local function firePurpleShot(player)
	...
	-- SFX 3D: charge — tocar no orbe vermelho (segue enquanto orbita)
	do
		local redPart = red and (red:IsA("Model") and (red.PrimaryPart or red:FindFirstChildWhichIsA("BasePart")) or (red:IsA("BasePart") and red))
		if redPart then
			SFXHelper.playAt(redPart, PURPLE_CHARGE_SFX_ID, 0.85, {
				minDist = 15, maxDist = 90, lifetime = chargeDuration + pauseBeforeMerge + 1, follow = true,
			})
		end
	end
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		if elapsed >= chargeDuration + pauseBeforeMerge then
			...
			-- SFX 3D: tocar no projétil roxo em movimento, com fade-out após 2.5s
			if handle and handle.Instance then
				local projInst = handle.Instance
				local projPart = projInst:IsA("Model") and (projInst.PrimaryPart or projInst:FindFirstChildWhichIsA("BasePart")) or (projInst:IsA("BasePart") and projInst)
				if projPart then
					SFXHelper.playAt(projPart, PURPLE_FIRE_SFX_ID, 0.9, {
						minDist      = 15,
						maxDist      = 100,
						lifetime     = 4,
						fadeStart    = 2.5,
