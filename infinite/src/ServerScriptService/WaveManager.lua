...
-- Load centralized enemy configuration
local ScriptsFolder = ReplicatedStorage:WaitForChild("Scripts")
local EnemyStats = require(ScriptsFolder:WaitForChild("EnemyStats"))
local EnemyModels = require(ScriptsFolder:WaitForChild("EnemyModels"))
...
-- Attach the correct AI script based on enemy TYPE (melee, ranged, regen, cloner)
-- Uses scripts from ReplicatedStorage.Scripts.Enemy_scripts/
local function attachEnemyAIByType(enemy, enemyType)
    if not enemy or not enemyType then return end
    if enemy:FindFirstChild("AI") then 
        -- Remove existing AI script (from model template) so we can apply the correct one
        enemy:FindFirstChild("AI"):Destroy()
    end
    
    local scriptsFolder = ReplicatedStorage:FindFirstChild("Scripts")
    local enemyScriptsFolder = scriptsFolder and scriptsFolder:FindFirstChild("Enemy_scripts")
    if not enemyScriptsFolder then 
        warn("[WaveManager] Enemy_scripts folder not found")
        return 
    end
    
    -- Map enemy type to AI script name
    local aiScriptName = enemyType:lower() .. "_AI"
    local aiScript = enemyScriptsFolder:FindFirstChild(aiScriptName)
    
    if aiScript and aiScript:IsA("Script") then
        local clone = aiScript:Clone()
        clone.Name = "AI"
        clone.Parent = enemy
        print("[WaveManager] Attached", aiScriptName, "to", enemy.Name)
    else
        warn("[WaveManager] AI script not found:", aiScriptName, "for type:", enemyType)
    end
end
...
-- Get base stats from centralized EnemyStats by TYPE
local function getEnemyBaseStatsByType(enemyType)
    local stats = EnemyStats.GetByType(enemyType)
    if not stats then
        warn("[WaveManager] No stats found for enemy type:", enemyType)
    end
    return stats
end
...
function WaveManager.new(config)
    ...
    if not loadedCfg then
        -- Only warn if not in infinite mode (infinite mode generates waves dynamically)
        if not self.Config.InfiniteMode then
    ...
    -- Store WaveConfig reference for infinite mode
    self.WaveConfig = loadedCfg
    ...
    if humanoid then
        humanoid.Died:Connect(function()
            if self._activeEnemies[enemy] then
                ...
                if debugWaves then print(string.format("[WaveManager] OnEnemyDied fired for type=%s id=%s wave=%d", 
                    tostring(meta and meta.type or "legacy"), tostring(meta and meta.id), self._waveIndex)) end
...
function WaveManager:_nextWave()
    ...
    -- INFINITE MODE: Generate waves procedurally if beyond pre-generated
    if self._waveIndex > #self.Waves then
        -- Check if WaveConfig has infinite mode enabled
        local waveConfig = self.WaveConfig
        if waveConfig and waveConfig.InfiniteMode then
            -- Check if we should repeat a pattern (waves 51-60)
            if waveConfig.RepeatWavesStart and waveConfig.RepeatWavesEnd then
                local repeatStart = waveConfig.RepeatWavesStart
                local repeatEnd = waveConfig.RepeatWavesEnd
                local repeatRange = repeatEnd - repeatStart + 1
                
                -- Calculate which wave in the repeat cycle (51-60)
                local cyclePosition = ((self._waveIndex - repeatEnd - 1) % repeatRange) + repeatStart
                
                -- Clone the wave from the repeat pattern
                local templateWave = self.Waves[cyclePosition]
                if templateWave then
                    local newWave = { enemies = {} }
                    for _, enemy in ipairs(templateWave.enemies or {}) do
                        table.insert(newWave.enemies, {
                            id = enemy.id,
                            count = enemy.count,
                            position = enemy.position,
                            positions = enemy.positions,
                            area = enemy.area
                        })
                    end
                    table.insert(self.Waves, newWave)
                    print(string.format("[WaveManager] Infinite mode: repeating wave pattern %d (cycle from wave %d)", self._waveIndex, cyclePosition))
                end
            elseif type(waveConfig.GenerateWave) == "function" then
                -- Fallback: generate procedurally
                local newWave = waveConfig.GenerateWave(self._waveIndex)
                table.insert(self.Waves, newWave)
                print(string.format("[WaveManager] Infinite mode: generated wave %d", self._waveIndex))
            end
        else
            -- Normal mode: all waves completed
            self._running = false
    ...
    task.spawn(function()
        local spawnIdx = 0
        for _, entry in ipairs(wave.enemies or {}) do
            -- Support both TYPE-based (new system) and ID-based (legacy) spawning
            local enemyType = entry.type -- New system: "melee", "ranged", etc.
            local enemyId = entry.id     -- Legacy: specific model name
            
            -- If TYPE is provided, choose a random model for that type
            local actualId
            if enemyType then
                actualId = EnemyModels.GetRandomModel(enemyType)
                if not actualId then
                    warn("[WaveManager] Failed to get random model for type:", enemyType)
                    actualId = enemyId or "Enemy"
                end
            else
                actualId = enemyId or "Enemy"
            end
            ...
            while spawnedForEntry < count do
                ...
                for _, p in ipairs(planned) do
                    ...
                    -- Load template and spawn enemy
                    local template = self.LoadEnemy and self.LoadEnemy(actualId) or nil
                    local enemy = spawnEnemyFromTemplate(template, p.cf)
                    
                    -- Apply stats based on TYPE (new system) or fall back to ID-based stats
                    local baseStats
                    if enemyType then
                        baseStats = getEnemyBaseStatsByType(enemyType)
                    else
                        baseStats = nil -- Legacy path, stats are in the model
                    end
                    applyEnemyBaseStats(enemy, baseStats)
                    
                    -- Apply wave-specific stats if provided (for procedural infinite mode)
                    if entry.baseHP then
                        local hum = enemy:FindFirstChildOfClass("Humanoid")
                        if hum then
                            hum.MaxHealth = entry.baseHP
                            hum.Health = entry.baseHP
                        end
                        enemy:SetAttribute("BaseHP", entry.baseHP)
                    end
                    if entry.baseDamage then
                        enemy:SetAttribute("BaseDamage", entry.baseDamage)
                        enemy:SetAttribute("Damage", entry.baseDamage)
                    end
                    if entry.xp then
                        enemy:SetAttribute("XPDrop", entry.xp)
                    end
                    ...
                    pcall(function()
                        ...
                        enemy:SetAttribute("EnemyId", actualId)
                        if enemyType then
                    ...
                    local meta = { id = actualId, type = enemyType, spawnIndex = p.sidx }
                    self:_trackEnemy(enemy, meta)
                    
                    -- Attach AI based on TYPE (new system) or ID (legacy)
                    if enemyType then
                        pcall(function() attachEnemyAIByType(enemy, enemyType) end)
                    else
                        -- Legacy: try to find AI in model folder
                    ...
                    if debugWaves then
                        local alive2 = 0
                        for _ in pairs(self._activeEnemies) do alive2 += 1 end
                        print(string.format("[WaveManager] spawned enemy type=%s model=%s wave=%d spawnIndex=%d spawnedInWave=%d active=%d",
                            tostring(enemyType or "legacy"), tostring(actualId), self._waveIndex, p.sidx, self._spawnedInWave, alive2))
                    end
