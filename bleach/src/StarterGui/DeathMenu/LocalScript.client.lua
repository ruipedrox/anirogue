...
local buyReviveRemote = remotesFolder and remotesFolder:FindFirstChild("DeathMenuBuyRevive") or nil
...
reviveBtn.MouseButton1Click:Connect(function()
	-- If a server-side purchase prompt is available, request it; otherwise attempt normal revive.
	if buyReviveRemote then
		buyReviveRemote:FireServer()
		print("[DeathMenu] Revive clicked -> request purchase (server)")
	else
		if reviveRemote then reviveRemote:FireServer() end
		print("[DeathMenu] Revive clicked -> server revive")
	end
