...
-- Server-side Remote for prompting a revive purchase (Developer Product)
...
DeathMenuBuyRevive.OnServerEvent:Connect(function(player)
	...
	-- Mark player as pending a revive purchase so defeat monitor won't end run
	lastAliveTimestamp = os.clock()
	pcall(function() player:SetAttribute("_PendingRevivePurchase", true) end)
	-- Prompt the client purchase UI for the Dev Product (revive)
	local ok, err = pcall(function()
		MarketplaceService:PromptProductPurchase(player, 3553813106)
	end)
	if not ok then
...
-- Lightweight ProcessReceipt: handle revive product purchases locally so players in this Place get revived immediately.
-- Note: This is intentionally minimal: it grants immediate respawn in-place for ProductId 3553813106.
-- ProcessReceipt handler will be assigned after serverDoRevive is defined to ensure revive logic is available.
...
-- Forward-declare serverDoRevive so ProcessReceipt can reference it regardless of order
local serverDoRevive
...
-- Grace window before considering defeat when all players are down
local DEFEAT_GRACE_SECONDS = 3
-- Seconds of invincibility granted after a server-side revive
local REVIVE_INVINCIBILITY_SECONDS = 3
-- Timestamp of last observed alive player; initialize to now to avoid nil comparisons
...
task.spawn(function()
	...
	while not runEnded and defeatMonitorThread == myToken do
		...
		for _, plr in ipairs(Players:GetPlayers()) do
			-- If player has a pending revive purchase, treat them as effectively alive
...
-- Handlers DeathMenu
serverDoRevive = function(player)
	if runEnded then return end
	...
	-- Trigger respawn (safe to call even if player already alive; CharacterAdded hook will handle restore)
	pcall(function() player:LoadCharacter() end)
end

-- Assign ProcessReceipt now that serverDoRevive exists
MarketplaceService.ProcessReceipt = function(receiptInfo)
	local productId = receiptInfo.ProductId
	local playerId = receiptInfo.PlayerId
	print(string.format("[ProcessReceipt][Village] Received receipt: ProductId=%s PlayerId=%s PurchaseId=%s", tostring(productId), tostring(playerId), tostring(receiptInfo.PurchaseId)))
	if productId ~= 3553813106 then
		print("[ProcessReceipt][Village] ProductId not handled here")
		return Enum.ProductPurchaseDecision.PurchaseGranted
	end
	local player = Players:GetPlayerByUserId(playerId)
	if not player then
		print("[ProcessReceipt][Village] Player not present on this server; will retry later")
		return Enum.ProductPurchaseDecision.NotProcessedYet
	end
	-- Proceed to revive player now
	local ok, err = pcall(function()
		print("[ProcessReceipt][Village] Reviving player now:", player.Name)
		serverDoRevive(player)
	end)
	if not ok then
		warn("[ProcessReceipt][Village] serverDoRevive failed:", err)
		return Enum.ProductPurchaseDecision.NotProcessedYet
	end
	-- Clear pending-purchase flag if set
	pcall(function() player:SetAttribute("_PendingRevivePurchase", false) end)
	print("[ProcessReceipt][Village] Processed revive purchase for", player.Name)
	return Enum.ProductPurchaseDecision.PurchaseGranted
end
